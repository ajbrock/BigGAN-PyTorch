...
class Generator(nn.Module):
  def __init__(self, n_classes=1000, G_ch=64, G_shared=True,dim_z=128,
    ...
    self.GBlock = nn.ModuleList([
      GBlock(16 * self.ch, 16 * self.ch, n_class=n_classes, z_dim=dim_z),
      GBlock(16 * self.ch, 8 * self.ch,  n_class=n_classes, z_dim=dim_z),
      GBlock(8 * self.ch, 4 * self.ch,   n_class=n_classes, z_dim=dim_z),
      GBlock(4 * self.ch, 2 * self.ch,   n_class=n_classes, z_dim=dim_z),
      GBlock(2 * self.ch, 1 * self.ch,   n_class=n_classes, z_dim=dim_z),
    ])
    ...
    self.ScaledCrossReplicaBN = nn.BatchNorm2d(1 * self.ch, eps=1e-4)
