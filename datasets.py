...
class Kinetics600(Dataset):
	...
	self.length = len(self.kinetics_hash)
    ...
    def __getitem__(self, index):
	vid_idx = index//121
	frame_idx = index-(121*vid_idx)

	frame = self.video_list[list(self.video_list.keys())[vid_idx]][frame_idx]
	label = self.kinetics_hash[self.video_list.keys())[vid_idx]]
